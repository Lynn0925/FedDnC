...
def extend_cfg(cfg):
    ...
    cfg.TRAINER.FEDNC = CN()
    cfg.TRAINER.FEDNC.N_CTX = 16
    cfg.TRAINER.FEDNC.CSC = False
    cfg.TRAINER.FEDNC.CTX_INIT = ""
    cfg.TRAINER.FEDNC.PREC = "fp16"
    cfg.TRAINER.FEDNC.CLASS_TOKEN_POSITION = "end"
    cfg.TRAINER.FEDNC.N = 2
    cfg.TRAINER.FEDNC.THRESH = 0.001
    cfg.TRAINER.FEDNC.EPS = 0.1
    cfg.TRAINER.FEDNC.OT = "COT"
    cfg.TRAINER.FEDNC.TOP_PERCENT = 0.8
    cfg.TRAINER.FEDNC.MAX_ITER = 100
    
    # FedDnC core hyperparameters
    cfg.TRAINER.FEDNC.LAMBDA_KD = 1.0
    cfg.TRAINER.FEDNC.BETA_DIV = 0.5
    cfg.TRAINER.FEDNC.ALPHA_ENSEMBLE = 0.6
    cfg.TRAINER.FEDNC.MU_PROX = 0.01
    cfg.TRAINER.FEDNC.EMA_ALPHA = -1.0  # -1 = use shots-based formula
    cfg.TRAINER.FEDNC.KD_TEMP = 4.0
    # Config for FEDPGP
    ...
    cfg.DATASET.TARGET_DOMAIN = ""  # LODO: leave-one-domain-out target domain (e.g. "clipart")
...
def setup_cfg(args):
    ...
    if args.opts and "DATASET.SUBSAMPLE_CLASSES" in args.opts:
        idx = args.opts.index("DATASET.SUBSAMPLE_CLASSES")
        val = args.opts[idx + 1]
        args.opts.pop(idx)
        args.opts.pop(idx)
        cfg.defrost()
        cfg.DATASET.SUBSAMPLE_CLASSES = val
        cfg.freeze()
        print(f"==> Manually set DATASET.SUBSAMPLE_CLASSES to {val}")
...
def main(args):
    ...
    print(f"ACTUAL TYPE: {type(federated_learner)}")  
