...
def prepare_data_domainNet(cfg, data_base_path):
    ...
    test_set = [infograph_trainset+infograph_testset]*6
    return train_set, test_set, classnames, lab2cname


def prepare_data_domainNet_lodo(cfg, data_base_path):
    """Leave-One-Domain-Out: 5 source clients (1 per domain), test on target domain (full data)."""
    all_domains = ['clipart', 'infograph', 'painting', 'quickdraw', 'real', 'sketch']
    target = getattr(cfg.DATASET, 'TARGET_DOMAIN', 'clipart').lower()
    assert target in all_domains, f"TARGET_DOMAIN must be one of {all_domains}, got '{target}'"
    ...
    lab2cname, classnames = None, None
    train_set = []
    for d in source_domains:
        ds = DomainNetDataset(data_base_path, d, transform=transform_train)
        if lab2cname is None:
            lab2cname = ds.lab2cname
            classnames = ds.classnames
        train_set.append(ds.data_detailed)

    # "whole target domain" = train split + test split combined
    target_data = (DomainNetDataset(data_base_path, target, transform=transform_test).data_detailed +
                   DomainNetDataset(data_base_path, target, transform=transform_test, train=False).data_detailed)
    test_set = [target_data] * len(source_domains)

    print(f"[LODO] target={target}, source={source_domains}")
    print(f"train_data_num_list: {[len(t) for t in train_set]}")
    print(f"target test size: {len(target_data)}")
    return train_set, test_set, classnames, lab2cname
...
def prepare_data_office_lodo(cfg, data_base_path):
    """Leave-One-Domain-Out: 3 source clients (1 per domain), test on target domain (full data)."""
    all_domains = ['amazon', 'caltech', 'dslr', 'webcam']
    target = getattr(cfg.DATASET, 'TARGET_DOMAIN', 'amazon').lower()
    assert target in all_domains, f"TARGET_DOMAIN must be one of {all_domains}, got '{target}'"
    source_domains = [d for d in all_domains if d != target]
    ...
    train_set = []
    for d in source_domains:
        ds = OfficeDataset(data_base_path, d, transform=transform_train)
        if lab2cname is None:
            lab2cname = ds.lab2cname
            classnames = ds.classnames
        train_set.append(ds.data_detailed)
    ...
    # whole target domain = train split + test split combined
    target_data = (OfficeDataset(data_base_path, target, transform=transform_test).data_detailed +
                   OfficeDataset(data_base_path, target, transform=transform_test, train=False).data_detailed)
    test_set = [target_data] * len(source_domains)
...
def prepare_data_office(cfg, data_base_path):
    ...
    test_set = [amazon_trainset + amazon_testset+ caltech_trainset + caltech_testset+dslr_trainset + dslr_testset]*4
    return train_set, test_set, classnames, lab2cname
...
def prepare_data_office_partition_train(cfg, data_base_path):
    ...
    K = 10
    min_require_size = 2
    n_clients = 5  # 5 clients per domain, 20 total
    # amazon
    print("Amazon: ")
...
def Dataset_partition_office(base_path, site, beta, split_test=True, n_parties=5, min_require_size=2):
    min_size = 0
    K = 10
    # np.random.seed(2023)
...
def Adjust_test_dataset_domainnet(site, class_ratio, data_root="DATA/DomainNet"):
    c_num = 10
    label_dict = {'bird':0, 'feather':1, 'headphones':2, 'ice_cream':3, 'teapot':4, 'tiger':5, 'whale':6, 'windmill':7, 'wine_glass':8, 'zebra':9}
    _, test_text_labels = np.load('{}/{}_test.pkl'.format(data_root, site), allow_pickle=True)
    test_labels = np.asarray([label_dict[text] for text in test_text_labels])
