...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if ctx_init:
            ...
            with torch.no_grad():
                embedding = clip_model.token_embedding(prompt.to(clip_model.token_embedding.weight.device)).type(dtype)
...
class FedPGP(TrainerX):
    ...
    def model_inference(self, input):
        """Override to handle multiple feature outputs, returning only logits."""
