...
from data_utils import prepare_data_domainNet, prepare_data_domainNet_lodo, prepare_data_domainNet_partition_train, prepare_data_domainNet_partition_client_train
...
class DomainNet():
    ...
    def __init__(self, cfg):
        ...
        target_domain = getattr(cfg.DATASET, 'TARGET_DOMAIN', '')
        if target_domain:
            exp_folder = 'fed_domainnet_lodo'
            print(f"LODO mode: target={target_domain}")
            train_set, test_set, classnames, lab2cname = prepare_data_domainNet_lodo(cfg, root)
        elif cfg.DATASET.IMBALANCE_TRAIN:
