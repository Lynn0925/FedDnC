    ...
    def __init__(
        ...
        if dataset.federated_train_x:
            for idx in range(cfg.DATASET.USERS):
                ...
                elif cfg.DATASET.USERS == 1:
                    # Centralized mode: use config batch size to avoid OOM in
                    # per-sample methods (e.g. CoCoOp) where the full few-shot
                    # set as one batch would be too large.
                    batch_size = cfg.DATALOADER.TRAIN_X.BATCH_SIZE
                else:
                    batch_size = len(dataset.federated_train_x[idx])
        ...
        if dataset.federated_test_x:
            for idx in range(cfg.DATASET.USERS):
                ...
                if not data_source:
                    # No neighbors (e.g., USERS=1 centralized mode); skip loader
                    fed_test_neighbor_loader_x_dict[idx] = None
                    continue
